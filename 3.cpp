...
#define GPA_INCREASE_VALUE 1.05
#define FIRST_OUTPUT_FILE_NAME "./semester1.sched"
#define NUMBER_OF_COUNT_ELEMENT_IN_OUTPUT_FILE_NAME 10
...
void find_semester_schedule_for_every_term(CourseTable course_table_info, CourseGrade course_grade_info);
Prerequisites find_passed_course_id(CourseGrade course_grade_info);
CourseTable find_semester_schedule(CourseTable course_table_info, CourseGrade course_grade_info, CourseTable all_takable_courses, long double semester_gpa);
...
CourseTable find_all_takable_courses(CourseTable course_table_info, CourseGrade course_grade_info, Prerequisites passed_courses_id);
...
void print_result(CourseTable takable_courses, string output_file_name);
Prerequisites add_new_semester_schedule_course_id_to_passed_courses_vector_id(Prerequisites passed_courses_id, CourseTable semester_schedule);
...
int main(int argc, char* argv[])
{
	...
	find_semester_schedule_for_every_term(course_table_info, course_grade_info);
	return 0;
}
...
void find_semester_schedule_for_every_term(CourseTable course_table_info, CourseGrade course_grade_info)
{
	...
	string output_file_name = FIRST_OUTPUT_FILE_NAME;
	int file_count = 1;
	while (passed_courses_id.size() < course_table_info.size())
	{
		CourseTable all_takable_courses = find_all_takable_courses(course_table_info, course_grade_info, passed_courses_id);
		CourseTable semester_schedule = find_semester_schedule(course_table_info, course_grade_info, all_takable_courses, semester_gpa);
		semester_schedule = generate_output(semester_schedule, 0, semester_schedule.size());
		print_result(semester_schedule, output_file_name);
		passed_courses_id = add_new_semester_schedule_course_id_to_passed_courses_vector_id(passed_courses_id, semester_schedule);
		semester_gpa *= GPA_INCREASE_VALUE;
		file_count++;
		output_file_name[NUMBER_OF_COUNT_ELEMENT_IN_OUTPUT_FILE_NAME] = (file_count + 48);
	}
}
...
void print_result(CourseTable takable_courses, string output_file_name)
{
	...
	for (int current = 0; current < takable_courses.size(); current++)
		output_file << takable_courses[current].id << endl;
}

Prerequisites add_new_semester_schedule_course_id_to_passed_courses_vector_id(Prerequisites passed_courses_id, CourseTable semester_schedule)
{
	for (int current = 0;current < semester_schedule.size();current++)
		passed_courses_id.push_back(semester_schedule[current].id);
	return passed_courses_id;
}